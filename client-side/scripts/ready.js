	...
	form.on('submit', function(e){
		...
		$.ajax(url_base + "/go.php", {
			...
			success: function(json_obj, status, jqXHR){
				...
				$('#box').append(go_obj.makeCompactDiv());
				refresh();
				////////////
			}, 
...
var refresh = function(){
	$('.mb').on('click', function(){
		...
		var refresh_div = $(this).parent();
		var go = refresh_div.data('go');
		$.ajax(url_base + "/go.php/" + go.id, {
			type: "GET",
			dataType: "json",
			success: function(json_obj, status, jqXHR){
				var obj = new refresh_result(json_obj);
				if(refresh_div.hasClass('showing')){
					refresh_div.children('.cur_price').remove();
					refresh_div.removeClass('showing');
				}
				var random = $('<div class="cur_price">Lowest Airfare Price: $' + obj.rand +'</div>');
				refresh_div.append(random);
				refresh_div.addClass('showing');
				if (obj.result == true){
					var under = $('<div class=".under"></div>');
					under.append('Notification email sent');
					under.hide()
					refresh_div.append(under);
					under.slideDown([1000]);
					this_button.remove();
					random.css("border", "1px solid green");
				}
				random.css("background-color", "none")
			},
			error: function(jqXHR, status, error){
				alert(jqXHR.responseText);
			}
		});
		refresh_div.children('.cur_price').css("border", "1px solid red").css("background-color", "#aaa");
	});
};

var refresh_result = function(json_obj){
	this.result = json_obj.result;
	this.rand = json_obj.rand;
}
